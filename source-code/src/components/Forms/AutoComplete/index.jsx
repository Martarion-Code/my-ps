...
const MainAutoComplete = ({
  ...
  const fetchInTheFirstLoad = async (value) => {
    ...
    isFunctionInObject(other, "onSelect") &&
      isCallSelectOnTheFirstLoad &&
      other.onSelect("", opt);
  };
  useEffect(() => {
    if (!!other.value) {
  ...
  const fetchData = async (value) => {
    const params = new URLSearchParams({
      ...
      .then(async (res) => {
        ...
        const response = get(resJson, responsePath, []) || [];
        const optionsMap = response.map((item, index) => ({
          key: "ac-" + index,
          label:
            item[labelPropName] +
            (!!postfixLabelPropName ? ` (${item[postfixLabelPropName]})` : ""),
  ...
  const onChangeHandler = (val, opt) => {
    ...
    if (isPayloadCallbackExist) {
      payloadCallback(
        (options || [])
          .map(({ payload }) => payload)
          .find((item) => item[valuePropName] === val)
      );
    }
  };
  useEffect(() => {
    ...
    <>
      <AutoComplete
        ...
        value={
          ...
          (isFreeText ? other.value : undefined)
        }
        allowClear
